...
<div class="single_product">
					...
					<div class="order_info d-flex flex-row">
						<form id ="myCart" action="{{route('frontend.post_shopping_cart')}}" method="POST" >
							<div class="clearfix" style="z-index: 1000;">
								...
								<ul class="product_color">
									<li>
										...
										<ul class="color_list">
											@foreach($product->productDetail as $item)
											<li><div class="color_mark" style="background: #{{$item->color->code}};" data-color_name="{{$item->color->name}}" data-product="{{$item->id}}" data-quantity="{{$item->quantity}}" data-price="{{$item->price}}" data-color_id="{{$item->color->id}}"></div></li>
											@endforeach
										</ul>
									</li>
							...
							<div class="stock" style="color: grey; font-size: 18px;">
								...
								<a id="stock_quantity" style="font-style: italic; font-size: 15px;">Vui lòng chọn màu</a>
							</div>
	...
	<div class="container" style="margin-top: 55px; margin-bottom: 15px">
		<ul class="nav nav-tabs" role="tablist" style="font-size: 20;">
			<li class="nav-item">
				<a class="nav-link active" href="#profile" role="tab" data-toggle="tab">Thông số</a>
			</li>
			<!-- <li class="nav-item">
		...
		<!-- 	<li class="nav-item">
			...
			<li class="nav-item">
				<a class="nav-link" href="#references" role="tab" data-toggle="tab">Đánh giá của người dùng</a>
			</li>
			<li class="nav-item">
				<a class="nav-link" href="#feedback" role="tab" data-toggle="tab">Nhận xét</a>
			</li>
		</ul>
				...
				<div class="row" style="margin-top: 35px;">
					<div class="col-lg-12">
						<table class="table table-bordered">
							<thead>
								<tr>
									<th scope="col" colspan="2" class="text-center">Thông số</th>
								</tr>
							</thead>
							<tbody>
					...
					<div class="widget-area no-padding blank">
						<div class="status-upload">
							...
							<form id="myFeedback" method="POST">
								...
								<!-- Rating Stars Box -->
								<textarea name="content" placeholder="Nhận xét của bạn về {{$product->name}}?" style="    margin-top: 18px;"></textarea>
...
<script type="text/javascript">
	...
	$('.color_mark').click(function(){
		...
		$('#product_detail_id').val($(this).attr('data-product'));
		$('#stock_quantity').empty();
		$('#stock_quantity').append($(this).attr('data-quantity') + ' products');
		$('.product_price').empty();
		$('.product_price').append('$' + $(this).attr('data-price'));
	...
	$('#selected_color').click(function(){
		...
		$('#stock_quantity').empty();
		$('#stock_quantity').append('Chosse color');
