...
class ProductRepository implements ProductRepositoryInterface
{
    ...
    public function listProduct($query, $url, $paginateQuery)
    {
        ...
        if (!empty($query['category'])) {
            if ($this->Category->find($query['category'])->parent_id) {
                $listProduct = $listProduct->where('category_id', $query['category']);
            } else {
                $listCategories = $this->Category->where('parent_id', $query['category'])->pluck('id')->toArray();
                $listProduct = $listProduct->whereIn('category_id', $listCategories);
            }
        }
        ...
        if (!empty($query['ram'])) {
            ...
            $listProduct = $listProduct->whereHas('specification', function ($listProduct) use ($query) {
                ...
                $listProduct->whereIn('ram', $query['ram']);
            });
        }
        ...
        if (!empty($query['rom'])) {
            ...
            $listProduct = $listProduct->whereHas('specification', function ($listProduct) use ($query) {
                ...
                $listProduct->whereIn('rom', $query['rom']);
            });
        }
        ...
        if (!empty($query['operator'])) {
            ...
            $listProduct = $listProduct->whereHas('specification', function ($listProduct) use ($query) {
                ...
                $listProduct->whereIn('os', $query['operator']);
            });
        }

        if (!empty($query['display'])) {

            $listProduct = $listProduct->whereHas('specification', function ($listProduct) use ($query) {

                $listProduct->whereIn('monitor', $query['display']);
            });
        }
        ...
        if (!empty($query['front_camera'])) {
            ...
            $listProduct = $listProduct->whereHas('specification', function ($listProduct) use ($query) {
                ...
                $listProduct->whereIn('front_camera', $query['front_camera']);
            });
        }

        if (!empty($query['rear_camera'])) {

            $listProduct = $listProduct->whereHas('specification', function ($listProduct) use ($query) {

                $listProduct->whereIn('rear_camera', $query['rear_camera']);
            });
        }

        if (!empty($query['battery'])) {

            $listProduct = $listProduct->whereHas('specification', function ($listProduct) use ($query) {

                $listProduct->whereIn('pin', $query['battery']);
            });
        }
        ...
        foreach ($listProduct as $product) {
            ...
            switch ($product->status) {
                case '1':
                    ...
                    $product->statusAdmin = ['number' => 1, 'text' => 'Đang kinh doanh', 'code' => 'btn-xs btn-success'];
                    break;
                case '2':
                    $product->status = ['number' => 2, 'text' => 'Sắp ra mắt', 'code' => '#e5800e'];
                    $product->statusAdmin = ['number' => 2, 'text' => 'Sắp ra mắt', 'code' => 'btn-xs btn-warning'];
                    break;
                case '3':
                    $product->status = ['number' => 3, 'text' => 'Ngừng kinh doanh', 'code' => '#e50e0e'];
                    $product->statusAdmin = ['number' => 3, 'text' => 'Ngừng kinh doanh', 'code' => 'btn-xs btn-danger'];
                    break;
                default:
                    $product->status = ['number' => 4, 'text' => 'N/a', 'code' => '#0ee511'];
                    $product->statusAdmin = ['number' => 4, 'text' => 'N/a', 'code' => 'btn-xs btn-primary'];
                    break;
            }
    ...
    public function nhanTest()
    {
        $list = $this->Product->get();
        return $list;
    }
}
