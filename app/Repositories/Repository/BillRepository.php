...
class BillRepository implements BillRepositoryInterface
{
    ...
    public function getProductBill($array = [])
    {
        $listProductDetail = $this->ProductDetail->whereIn('product_id', $array)->get()->pluck('id');

        if (count($listProductDetail) <= 0) return true;
        
        $listBill = $this->BillDetail->whereIn('product_detail_id', $listProductDetail)->get()->pluck('bill_id');

        // dd($listBill);

        $listBill = array_unique($listBill->toArray());

        if (count($listBill) <= 0) return true;

        $resultBill = $this->Bill->whereIn('id', $listBill)->where('status', 1)->get();

        if (count($resultBill) <= 0) return true;
