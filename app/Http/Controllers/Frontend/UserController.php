...
class UserController extends Controller
{
	...
	public function postUserProfile(Request $request)
	{
		...
		if ($validator->fails()) {
			...
			return $this->dataSuccess('Cập nhật thông tin thành công!!!');
		}
	}
	...
	public function changePassword(Request $request)
	{
		if (!(Hash::check($request->currentpassword, Auth::user()->password))) {
			...
			$array[] = ['key' => 'currentpassword', 'mess' => ["Mật khẩu hiện tại không chính xác"]];
		...
		if (strcmp($request->currentpassword, $request->password) == 0) {
			...
			$array[] = ['key' => 'password', 'mess' => ["Mật khẩu mới và cũ giống nhau"]];
		...
		if ($validator->fails()) {
			...
			return $this->dataSuccess('Đỗi mật khẩu thành công');
		}
	}
	...
	public function postLogin(Request $request)
	{
		...
		$messages = [
			'email.required' => 'Vui lòng điền email hoặc tên đăng nhập',
		...
		if ($validator->fails()) {
			...
			if (Auth::attempt([$field => $request->email, 'password' => $request->password])) {
				...
				if ($this->user->checkUserAvailable($request->email)) {
					...
					$array[] = ['key' => 'email', 'mess' => ["Username và mật khẩu không hợp lệ .Bạn có muốn đăng ký không? "]];
	...
	public function postComment(Request $request, $product_id)
	{
		...
		if ($validator->fails()) {
			...
			if ($this->user->checkUserCommentAvailable($request->user()->id, $product_id)) {
				...
				return $this->dataError('Bạn đã đánh giá sản phẩm này rồi!!!', $array);
			}
			...
			if ($saveComment) {
				...
				return $this->dataSuccess('Đánh giá thành công!!!');
			} else {
